...
Audit the codebase across the categories in [improve-references/audit-playbook.md](improve-references/audit-playbook.md) — read it now. Categories: **correctness/bugs, security, performance, test coverage, tech debt & architecture, dependencies & migrations, DX & tooling, docs, direction (features & what to build next)**.
...
- the **absolute path** to this command's `improve-references/audit-playbook.md` plus the exact section headings to read — **always including "## Finding format"** (subagents can read files — this is far cheaper than pasting; paste the sections only if the path may not resolve in the subagent's environment),
...
For each selected finding, write one plan file using the template in [improve-references/plan-template.md](improve-references/plan-template.md) — read it before writing the first plan. Plans go in:
